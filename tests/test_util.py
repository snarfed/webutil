...
""", util.to_xml({'a': {'b': {'c': 'x', 'd': 'y'}, 'e': (2, 3), }}))
  ...
  def test_interpret_http_exception(self):
    ...
    for code in (400, 500):
      ...
      self.assertEqual(
        ('503', body.getvalue()),
        ihc(HTTPError('url', code, 'BAD REQUEST', {}, io.BytesIO(body.encode()))))
    ...
    self.assertEqual(('504', 'foo bar'), ihc(socket.timeout(9, 'foo bar')))
