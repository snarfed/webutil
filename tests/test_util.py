...
""", util.to_xml({'a': {'b': {'c': 'x', 'd': 'y'}, 'e': (2, 3), }}))
  ...
  def test_get_list(self):
    ...
    for input, expected in (
      ...
      self.assertEqual(expected, util.get_list(input, 0))
  ...
  def test_get_first(self):
    for input, expected in (
      ...
      self.assertEqual(expected, util.get_first(input, 0))
  ...
  def test_clean_url(self):
    ...
    for bad in None, 'http://foo]', 3.14, ['http://foo']:
      self.assertIsNone(util.clean_url(bad))  # type: ignore
  ...
  def test_interpret_http_exception(self):
    ...
    # rate limiting
    ex = HTTPError('url', 429, 'msg', {}, io.BytesIO(b'my body'))
    ...
    got, _ = ihc(HTTPError('url', 400, '', {}, json_str(body)))
    self.assertEqual('429', got)
    ...
    msg = 'message=Sorry, the Flickr API service is not currently available., flickr c# ode=0'
    self.assertEqual(('504', msg), ihc(HTTPError('url', 400, msg, {}, None)))

    # https://console.cloud.google.com/errors/13299057966731352169?project=brid-gy
    self.assertEqual(('504', 'Unknown'),
                     ihc(HTTPError('url', 418, 'Unknown', {}, None)))
    ...
    for code in (400, 500):
      body_io = json_str(fb_transient)
      self.assertEqual(
        ('503', body_io.getvalue()),
        ihc(HTTPError('url', code, 'BAD REQUEST', {}, body_io)))
    ...
    self.assertEqual(
      ('504', '[Errno -1] foo bar'),
      ihc(HTTPError('url', None, err, {}, None)))  # type: ignore

    # upstream connection failures are converted to 504
    self.assertEqual(('504', '[Errno 9] foo bar'),
                     ihc(socket.timeout(9, 'foo bar')))
  ...
  def test_is_connection_failure(self):
    for e in (
        ...
        URLError(socket.gaierror(9, 'foo bar')),
  ...
  def test_encode_decode_oauth_state(self):
    for bad in None, 1, [], (), 'x':
      with self.assertRaises(TypeError):
        util.encode_oauth_state(bad)  # type: ignore
