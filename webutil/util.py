...
TODO: fix bug in :attr:`LINK_RE` that makes it miss emoji domain links without
scheme, eg ``☕⊙.ws``. bug is that the ``\b`` at the beginning of
:attr:`SCHEME_RE` doesn't apply to emoji, since they're not word-constituent
characters, and that the ``?`` added in :attr:`LINK_RE` only applies to the
...
class CacheDict(dict):
  ...
  e.g. :meth:`granary.Source.get_activities_response`.
  """
...
def pop_list(obj, key):
  """Like :func:`get_list`, but also removes the item."""
...
def encode(obj, encoding='utf-8'):
  ...
  Returns:
    sequence or dict: obj with all unicode strings encoded
  """
...
def domain_from_link(url, minimize=True):
  ...
  Args:
    url (string)
    minimize (bool): if true, strips ``www.``, ``mobile.``, and ``m.`` subdomains from
      the beginning of the domain

  Returns:
    str: optional, ``None`` if url is ``None`` or blank
  """
  if not url:
    return None
...
def schemeless(url, slashes=True):
  ...
  Args:
    url (str)
    slashes (bool): if False, also strips leading slashes and trailing slash,
...
def clean_url(url):
  ...
  Returns:
    str: the cleaned url, or ``None`` if it can't be parsed
  """
  if not url:
    return url
...
def quote_path(url):
  ...
  Returns:
    str: the quoted url, or ``None`` if it can't be parsed
  """
  try:
    parts = list(urlparse(url))
  except (AttributeError, TypeError, ValueError):
...
def is_url(url):
  ...
  try:
    if re.search(r'\s', url):
      return False
    parsed = urlparse(url)
    return bool((parsed.scheme and parsed.netloc) or parsed.scheme == 'acct')
...
def tokenize_links(text, skip_bare_cc_tlds=False, skip_html_links=True,
  ...
  Args:
    text (str): to linkify
    skip_bare_cc_tlds (bool): whether to skip links of the form
      ``[domain].[2-letter TLD]`` with no schema and no path
    skip_html_links (bool): whether to skip links in HTML ``<a>`` tags
      (both href and text)
    require_scheme (bool): whether to require scheme (eg ``http://``)

  Returns:
    (sequence of str, sequence of str) tuple: list of links and list of non-link
    text. Roughly equivalent to the output of :func:`re.findall` and
    :func:`re.split`, with some post-processing.
  """
...
def linkify(text, pretty=False, skip_bare_cc_tlds=False, **kwargs):
  ...
  Args:
    text (str): input
    pretty (bool): if True, uses :func:`pretty_link()` for link text
    skip_bare_cc_tlds (bool): whether to skip links of the form
      ``[domain].[2-letter TLD]`` with no schema and no path
...
def pretty_link(url, text=None, text_prefix=None, keep_host=True,
  ...
  * https://dev.twitter.com/docs/counting-characters
  ...
  Args:
    ...
    glyphicon (str): glyphicon to render after the link text, if provided.
...
def parse_iso8601_duration(input):
  ...
  number of days in a month. Background: https://stackoverflow.com/a/29458514/186123
...
def to_iso8601_duration(input):
  ...
  Returns:
    str: ISO 8601 duration, e.g. ``P3DT4S``
...
def add_query_params(url, params):
  ...
  Args:
    url (str): URL or :class:`urllib.request.Request`. May already have query
      parameters.
...
def remove_query_param(url, param):
  ...
  Returns:
    (str, str) tuple: (URL without the given param, param value)
  """
  # convert to list so we can modify later
  parsed = list(urlparse(url))
...
def requests_fn(url, fn=None, *args, log_data=True, cache=False, **kwargs):
  ...
  try:
    ...
    if cache_key and cache_key in cache_dict:
      ...
      logger.debug(f'HTTP cache hit for {cache_key}')
...
def requests_post_with_redirects(url, *args, **kwargs):
  ...
  Raises:
    :class:`requests.TooManyRedirects`
  """
...
def follow_redirects(url, **kwargs):
  ...
  Args:
    url (str)
    kwargs: passed to :func:`requests.head`

  Returns:
    :class:`requests.Response`: from the final request. The ``url`` attribute has
      the final URL.
  """
  try:
...
class UrlCanonicalizer(object):
  ...
  def __init__(self, scheme='https', domain=None, subdomain=None, approve=None,
    ...
    Args:
      scheme (str): canonical scheme for this source (default 'https')
      domain (str): canonical domain for this source (default None). If set,
        links on other domains will be rejected without following redirects.
      subdomain (str): canonical subdomain, e.g. 'www' (default none, ie root
        domain). only added if there's not already a subdomain.
      approve (str): regexp matching URLs that are automatically considered
        canonical
      reject (str): regexp matching URLs that are automatically considered
        canonical
      query (bool): whether to keep query params, if any (default False)
      fragment (bool): whether to keep fragment, if any (default False)
      trailing slash (bool): whether the path should end in / (default False)
      redirects (bool): whether to make HTTP HEAD requests to follow
        redirects (default True)
      headers (dict): passed through to the requests.head call for following
        redirects
    """
...
def parse_html(input, **kwargs):
  ...
  Args:
    input: (str or :class:`requests.Response`): input HTML
  ...
  if isinstance(input, requests.Response):
    ...
    #
    # requests is working on incorporating that change, but hasn't shipped it yet.
...
def parse_mf2(input, url=None, id=None, metaformats=None):
  ...
  Args:
    input: (str, :class:`bs4.BeautifulSoup`, or :class:`requests.Response`)
...
def parse_http_equiv(content):
  ...
  Args:
    content (str): ``http_equiv`` content str:
      https://www.w3.org/TR/WCAG20-TECHS/H76.html#procedure

  Returns:
    str: empty if content format is incorrect
  """
...
def fetch_http_equiv(input, **kwargs):
  ...
  Returns:
    str: empty if not available or a url if available
  """
  if not isinstance(input, (bs4.BeautifulSoup, bs4.Tag)):
    input = parse_html(input)
